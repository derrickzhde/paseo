...
describe("ClaudeAgentClient.fetchCatalog", () => {
  ...
  test("returns hardcoded claude models", async () => {
    ...
    try {
      ...
      expect(models.find((model) => model.id === "claude-fable-5[1m]")?.isSelectable).not.toBe(
        false,
      );
  ...
  test("preserves the catalog when Claude Code version detection fails", async () => {
    ...
    try {
      ...
      expect(models.find((model) => model.isDefault)?.id).toBe("claude-opus-5-5[1m]");
  ...
  test("exposes Ultra Code on xhigh-capable Claude models", async () => {
    ...
    try {
      ...
      expect(getThinkingIds("claude-fable-5-1[1m]")).toContain("ultracode");
